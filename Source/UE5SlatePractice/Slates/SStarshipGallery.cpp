...
class SStarshipGallery: public SCompoundWidget
{
public:
...
public:
    ...
    void Construct( const FArguments& InArgs )
    {
        ...
        FGlobalTabmanager::Get()->RegisterNomadTabSpawner("Colors", FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs&) -> TSharedRef<SDockTab> 
        { 
           return SNew(SDockTab).TabRole(ETabRole::PanelTab).ContentPadding(0) 
               [ 
                   ConstructColorsGallery() 
               ]; 
        }));

        FGlobalTabmanager::Get()->RegisterNomadTabSpawner("Text", FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs&) -> TSharedRef<SDockTab> 
        { 
           return SNew(SDockTab).TabRole(ETabRole::PanelTab).ContentPadding(0) 
               [ 
        ...
        FGlobalTabmanager::Get()->RegisterNomadTabSpawner("Icons", FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs&) -> TSharedRef<SDockTab> 
        { 
           return SNew(SDockTab).TabRole(ETabRole::PanelTab).ContentPadding(0) 
               [ 
        ...
        FGlobalTabmanager::Get()->RegisterNomadTabSpawner("Starship Widgets", FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs&) -> TSharedRef<SDockTab> 
        { 
           return SNew(SDockTab).TabRole(ETabRole::PanelTab).ContentPadding(0).ForegroundColor(FSlateColor::UseStyle()) 
               [ 
                   ConstructWidgetGallery() 
               ]; 
       }));

		FGlobalTabmanager::Get()->RegisterNomadTabSpawner("List Widgets", FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs&) -> TSharedRef<SDockTab> 
        { 
           return SNew(SDockTab).TabRole(ETabRole::PanelTab).ContentPadding(0).ForegroundColor(FSlateColor::UseStyle()) 
               [ 
        ...
        /*
        FGlobalTabmanager::Get()->RegisterTabSpawner("SLATE WIDGETS", FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs&) -> TSharedRef<SDockTab>
        { 
    ...
    TSharedRef<SWidget> ConstructColorsGallery()
    {
        auto GenerateColorButton = [] (FText InTitle, FName StyleColorName, bool Inverted = false, bool Outline = false) -> TSharedRef<SWidget> 
        {
            ...
            return SNew(SBox).Padding(8).WidthOverride(120).HeightOverride(100)
            [ 
                SNew(SOverlay) 
                    
               +SOverlay::Slot() 
                [ 
                    SNew(SBorder).BorderImage(Outline ? new FSlateRoundedBoxBrush(InSlateColor, 6.f, OutlineColor, 1.0f) : new FSlateRoundedBoxBrush(InSlateColor, 6.0f))//.Color(InSlateColor.GetSpecifiedColor())
                ] 
               +SOverlay::Slot().Padding(12) 
                [ 
                    SNew(SVerticalBox) 

                   +SVerticalBox::Slot().FillHeight(1).VAlign(VAlign_Bottom) 
                    [ 
                        SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("SmallFontBold")).ColorAndOpacity( FAppStyle::Get().GetSlateColor( Inverted ? "Colors.Background": "Colors.White")).Text(InTitle)
                    ] 
                   +SVerticalBox::Slot().VAlign(VAlign_Bottom).AutoHeight() 
                    [ 
                        SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("SmallFont")).ColorAndOpacity( FAppStyle::Get().GetSlateColor( Inverted ? "Colors.Background" : "Colors.White")).Text(FText::FromString(InSlateColor.GetSpecifiedColor().ToFColor(true).ToHex()))
                    ] 
                ]
            ];
        };
        ...
        [ 
          ...
          + SScrollBox::Slot().Padding(48) 
            [ 
             ...
             +SVerticalBox::Slot().Padding(8.f, 24.f, 8.f, 8.f).AutoHeight() 
              [ 
    ...
    TSharedRef<SWidget> ConstructIconsGallery()
    {
        ...
        auto GenerateIconLibrary = [this] (FText InTitle, FString InPath) -> TSharedRef<SWidget>
        {
            ...
            TSharedPtr<SUniformWrapPanel> UniformWrapPanel = SNew(SUniformWrapPanel).HAlign(HAlign_Left).SlotPadding( FMargin(12.f, 12.f) );
            ...
            for (const FString& Filename : FoundIcons)
            {
                ...
                [
                    SNew(SImage).Image(DynamicBrushes.Last().Get()).ToolTipText(FText::FromString( IconPath)).Visibility_Lambda([this, IconPath]() { return IsIconVisible(IconPath) ? EVisibility::Visible : EVisibility::Collapsed; })
                ];
            }

            return SNew(SVerticalBox) 
                +SVerticalBox::Slot().AutoHeight() [ SNew(STextBlock).Text(InTitle) ]
                +SVerticalBox::Slot()[ UniformWrapPanel.ToSharedRef() ];
        };
        ...
        return SNew(SBorder).BorderImage(FAppStyle::Get().GetBrush("ToolPanel.GroupBorder"))
        [
            SNew(SScrollBox)
            + SScrollBox::Slot().Padding(48)
            [
                ...
                [
                    SAssignNew(IconSearchBox, SSearchBox).HintText(LOCTEXT("IconSearchHint", "Enter text to filter icons by path..."))
                ]
    ...
    TSharedRef<SWidget> ConstructTextGallery()
    {
        ...
        return SNew(SBorder).Padding(48.f)
        [
            SNew(SHorizontalBox)

            +SHorizontalBox::Slot().AutoWidth().Padding(64.f, 0.0f)
            [
                SNew(SVerticalBox)
                ...
                +SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight() [ SNew(STextBlock).Text(NSLOCTEXT("StarshipGallery", "NormalBodyText", "Normal Text\n\nThe quick brown fox jumps over the lazy dog.\n\nNORMAL TEXT CAPITAL") ) ]
                ...
                +SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight()[ SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("SmallFont")).Text(NSLOCTEXT("StarshipGallery", "SmallBodyText", "Small Text\n\nThe quick brown fox jumps over the lazy dog.\n\nSMALL TEXT CAPITAL") ) ]
            ]
            ...
            +SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight()[ SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("ExtraLargeFont")).Text(NSLOCTEXT("StarshipGallery", "ExtraLargeBodyText", "Extra Large Body Text\n\nThe quick brown fox jumps over the lazy dog.\n\nEXTRA LARGE TEXT CAPITAL") ) ]
            ...
            +SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight()[ SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("LargeFont")).Text(NSLOCTEXT("StarshipGallery", "LargeBodyText", "Large Text\n\nThe quick brown fox jumps over the lazy dog.\n\nLARGE TEXT CAPITAL") ) ]
            */
            ...
            [
                ...
                +SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight() [ SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("NormalFontBold")).Text(NSLOCTEXT("StarshipGallery", "NormalBodyBoldText", "Normal Text Bold \n\nThe quick brown fox jumps over the lazy dog.\n\nNORMAL TEXT BOLD CAPITAL"))]
                ...
                +SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight()[ SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("SmallFontBold")).Text(NSLOCTEXT("StarshipGallery", "SmallBodyBoldText", "Small Text Bold \n\nThe quick brown fox jumps over the lazy dog.\n\nSMALL TEXT BOLD CAPITAL") ) ]
            ]
			...
			[
				...
				+ SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight() [ SNew(STextBlock).Text(NSLOCTEXT("StarshipGallery", "MathematicalTextSpecimen", "Mathematical Alphanumeric Symbols, U+1D400 - U+1D7FF\n\U0001D400\U0001D401\U0001D402 \U0001D434\U0001D435\U0001D436 \U0001D4D0\U0001D4D1\U0001D4D2")) ]
				...
				+ SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight() [ SNew(STextBlock).Text(NSLOCTEXT("StarshipGallery", "EmoticonsTextSpecimen", "Emoticons, U+1F600 - U+1F64F\n\U0001F60E\U0001F643\U0001F648")) ]
				+ SVerticalBox::Slot().Padding(0.f, 4.f).AutoHeight() [ SNew(STextBlock).Text(NSLOCTEXT("StarshipGallery", "SupplementalSymbolsPictographsTextSpecimen", "Supplemental Symbols and Pictographs, U+1F900 - U+1F9FF\n\U0001F914\U0001F916\U0001F9F2")) ]
			]
        ];
    }
	...
	TSharedRef<SWidget> BuildToolbar(bool bIncludeSettingsButtons)
	{
        ...
        }), LOCTEXT("Dropdown","Dropdown"), FText::GetEmpty(), FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.box-perspective"));
		...
		if(bIncludeSettingsButtons)
		{
			...
			{
				Toolbar.AddToolBarButton(ButtonAction, NAME_None, LOCTEXT("ToolBarButton1", "Button"), FText::GetEmpty(), FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.box-perspective"));
				Toolbar.AddComboButton(FUIAction(), FOnGetContent::CreateLambda([&]() 
                { 
                    FMenuBuilder Menu(true, nullptr); 
                    MakeMenuEntries(Menu); 
                    return Menu.MakeWidget(); 
                }), TAttribute<FText>(), TAttribute<FText>(), TAttribute<FSlateIcon>(), true);
    ...
    TSharedRef<SWidget> ConstructWidgetGallery()
    {
		...
		auto NextSlot = [WidgetNum, RowCount, Cols](TSharedPtr<SGridPanel> Grid, const FText& InLabel) mutable -> SHorizontalBox::FScopedWidgetSlotArguments
		{
			...
			Grid->AddSlot((WidgetNum / RowCount) * Cols, WidgetNum % RowCount).Padding(12.f).HAlign(HAlign_Left).VAlign(VAlign_Center) 
            [ 
                SNew(SCheckBox)
			...
			// Add the Label
			Grid->AddSlot((WidgetNum / RowCount) * Cols + 1, WidgetNum % RowCount).Padding(24.f, 16.f, 12.f, 16.f).VAlign(VAlign_Center).HAlign(HAlign_Right) 
            [ 
                SNew(STextBlock).Font(FAppStyle::Get().GetFontStyle("NormalFont")).ColorAndOpacity(FAppStyle::Get().GetSlateColor("Colors.White50")).Text(InLabel)
            ];

			//auto& Ret = Grid->AddSlot((WidgetNum / RowCount)*2 + 1, WidgetNum % RowCount)
			Grid->AddSlot((WidgetNum / RowCount) * Cols + 2, WidgetNum % RowCount).Padding(12.f, 16.f, 12.f, 16.f).VAlign(VAlign_Center).HAlign(HAlign_Fill) 
            [ 
                HBox 
            ];
			...
			NewSlot.VAlign(VAlign_Center).HAlign(HAlign_Fill).AutoWidth();
        ...
        auto LeftRightLabel = [](const FName& InIconName = FName(), const FText& InLabel = FText::GetEmpty(), const FName& InTextStyle = TEXT("ButtonText")) -> TSharedRef<SWidget>
        {
        	...
        	if (!InIconName.IsNone())
        	{
	            ...
	            [
	                SNew(SImage).ColorAndOpacity(FSlateColor::UseForeground()).Image(FAppStyle::Get().GetBrush(InIconName))
	            ];
        	}

        	if (!InLabel.IsEmpty())
        	{
        		HBox->AddSlot().VAlign(VAlign_Center).Padding(Space, 0.5f, 0.f, 0.f).AutoWidth()
	            [
	                SNew(STextBlock).TextStyle(&FAppStyle::Get().GetWidgetStyle<FTextBlockStyle>(InTextStyle)).Justification(ETextJustify::Center).Text(InLabel)
	            ];
        	}
        ...
        [
            ...
            +SHorizontalBox::Slot().AutoWidth().Padding(8.f, 0.0f).VAlign(VAlign_Center)
            [
                SNew(SButton)
            ...
            +SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Center).Padding(8.f, 0.0f)
            [
                SNew(SButton)
                .ButtonStyle( &FAppStyle::Get().GetWidgetStyle< FButtonStyle >( "PrimaryButton" ) )
        ...
        [
            ...
            +SHorizontalBox::Slot().AutoWidth().Padding(8.f, 0.0f).VAlign(VAlign_Center)
            [
                SNew(SButton).ButtonStyle(&FAppStyle::Get().GetWidgetStyle<FButtonStyle>( "Button" ))
                [
                    LeftRightLabel(NAME_None, LOCTEXT("Label", "Label"), "SmallButtonText")
                ]
            ]
            +SHorizontalBox::Slot().AutoWidth().Padding(8.f, 0.0f)
            [
                SNew(SButton).ButtonStyle( &FAppStyle::Get().GetWidgetStyle< FButtonStyle >( "Button" ) )
                [
        ...
        [
            ...
            +SHorizontalBox::Slot().AutoWidth().Padding(8.f, 0.0f).VAlign(VAlign_Center)
            [
                SNew(SButton).ButtonStyle(&FAppStyle::Get().GetWidgetStyle<FButtonStyle>("SimpleButton"))
                [
                    LeftRightLabel(NAME_None, LOCTEXT("Label", "Label"), "SmallButtonText")
                ]
            ]
        ...
        [
            ...
            +SHorizontalBox::Slot().AutoWidth().Padding(8.f)
            [
                SNew(SCheckBox).Style( &FAppStyle::Get().GetWidgetStyle<FCheckBoxStyle>("ToggleButtonCheckBox"))
                [
        ...
        [
        	SNew(SSegmentedControl<int32>).Value(0) // InitialValue
            // .OnValueChanged_Lambda( [this] (int32 InValue) { SegmentedBoxChoice = InValue; } ) 
            // .Value_Lambda( [this] { return SegmentedBoxChoice; } )  // Bound 

        	+SSegmentedControl<int32>::Slot(0).Icon(FAppStyle::Get().GetBrush("Icons.box-perspective")).Text(LOCTEXT("Box", "Box"))
            +SSegmentedControl<int32>::Slot(1).Icon(FAppStyle::Get().GetBrush("Icons.cylinder")).Text(LOCTEXT("Cylinder", "Cylinder"))
            +SSegmentedControl<int32>::Slot(2).Icon(FAppStyle::Get().GetBrush("Icons.pyramid")).Text(LOCTEXT("Pyramid", "Pyramid"))
        	+SSegmentedControl<int32>::Slot(3).Icon(FAppStyle::Get().GetBrush("Icons.sphere")).Text(LOCTEXT("Sphere", "Sphere"))
        ];
        ...
        [
            SNew(SSegmentedControl<int32>).Value(2)
        ...
        [
            SNew(SComboBox<TSharedPtr<FString> >).ComboBoxStyle( FAppStyle::Get(), "SimpleComboBox").OptionsSource(&ComboItems).OnGenerateWidget_Lambda([](TSharedPtr<FString> Item) 
            { 
                return SNew(STextBlock).Text( FText::FromString(*Item));
            })
            [
                SNew(SImage).ColorAndOpacity(FSlateColor::UseForeground()).Image(FAppStyle::Get().GetBrush("Icons.box-perspective"))
            ]
        ];


        // SComboBox
        NextSlot(WidgetGrid, LOCTEXT("SComboBoxLabel", "ComboBox")).AutoWidth()//.FillWidth(1.0)
        [
            SNew(SComboBox<TSharedPtr<FString>>).OptionsSource(&ComboItems).OnGenerateWidget_Lambda([](TSharedPtr<FString> Item) 
            { 
                return SNew(STextBlock).Text( FText::FromString(*Item));
            })
        ...
        [
            ...
            [ 
                SNew(STextBlock).Text(LOCTEXT("HeaderContentLabel", "Header Content")) 
            ]
        ];
        ...
        [
            SNew(SHyperlink).Text(LOCTEXT("SHyperlinkText", "Text to appear in the Hyperlink widget."))
        ];
        ...
        [
          ...
          + SHorizontalBox::Slot().AutoWidth() 
            [ 
                SNew(SButton).Text(LOCTEXT("AddBreadCrumbLabel", "Add")).HAlign(HAlign_Center).VAlign(VAlign_Center).OnClicked(this, &SWidgetGallery::HandleBreadcrumbTrailAddButtonClicked)
            ]
        ];
        ...
        */
        ...
        [ 
            SNew(STextBlock).Text(LOCTEXT("SImageLabel", "SImage")) 
        ]

        WidgetGrid->AddSlot(1, 16).Padding(0.0f, 5.0f) 
        [ 
            SNew(SImage).Image(FTestStyle::Get().GetBrush(TEXT("NewLevelBlank")))           
        ]
       */
        ...
        [
          ...
          + SVerticalBox::Slot().AutoHeight() 
            [ 
                SNew(SProgressBar).Percent(this, &SWidgetGallery::HandleProgressBarPercent) 
            ]
        ...
        */
        ...
        [ 
            SNew(STextBlock).Text(LOCTEXT("SRotatorInputBoxLabel", "SRotatorInputBox")) 
        ]

        WidgetGrid->AddSlot(1, 19).Padding(0.0f, 5.0f) 
        [ 
            SNew(SRotatorInputBox).Roll(0.5f).Pitch(0.0f).Yaw(1.0f)          
        ]

       // SSeparator
        NextSlot(WidgetGrid, LOCTEXT("SSeparatorLabel", "SSeparator"))
        [
            SNew(SBox).HeightOverride(100.0f).WidthOverride(150.0f) 
            [ 
                SNew(SHorizontalBox) 

              + SHorizontalBox::Slot().VAlign(VAlign_Center).FillWidth(0.75f) 
                [ 
                    SNew(SSeparator).Orientation(Orient_Horizontal) 
                ]
              + SHorizontalBox::Slot().HAlign(HAlign_Center).FillWidth(0.25f) 
                [ 
        ...
        */
        ...
        // SSlider
        NextSlot(WidgetGrid, LOCTEXT("SSliderLabel", "SSlider")).FillWidth(1.0)
        [
            SNew(SSlider).Orientation(Orient_Horizontal).Value(0.5f)//.IndentHandle(false)

            /*
          + SHorizontalBox::Slot().HAlign(HAlign_Center).FillWidth(0.25f) 
            [ 
                SNew(SSlider).Orientation(Orient_Vertical).Value(0.5f) 
            ]
            */
        ];
        ...
        NextSlot(WidgetGrid, LOCTEXT("SSliderNoIndentLabel", "SSlider (no indentation)")).FillWidth(1.0)
        [
            SNew(SSlider).IndentHandle(false).Orientation(Orient_Horizontal).Value(0.5f)
        ];

        // SSpacer
        NextSlot(WidgetGrid, LOCTEXT("SSpacerLabel", "SSpacer"))
        [
            SNew(SSpacer).Size(FVector2D(100, 100))      
        ];

        // SSpinningImage
        WidgetGrid->AddSlot(0, 25) 
        [ 
            SNew(STextBlock).Text(LOCTEXT("SSpinningImageLabel", "SSpinningImage")) 
        ]

        WidgetGrid->AddSlot(1, 25).HAlign(HAlign_Left).Padding(0.0f, 5.0f) 
        [ 
            SNew(SSpinningImage).Image(FTestStyle::Get().GetBrush("TestRotation16px"))
        ]
        */

        // SSpinBox
        NextSlot(WidgetGrid, LOCTEXT("SSpinBoxLabel", "SSpinBox")).FillWidth(1.0)
        [
            SNew(SBox).MinDesiredWidth(220)
            [
                SNew(SSpinBox<float>).MinValue(0.0f).MaxValue(500.0f).MinSliderValue(TAttribute< TOptional<float> >(-500.0f)).MaxSliderValue(TAttribute<TOptional<float>>(500.0f)).Value(123.0456789).Delta(0.5f)
            ]
        ];

        // SNumericEntrySpinBox
        NextSlot(WidgetGrid, LOCTEXT("SNumericEntryBoxLabel", "SNumericEntryBox")).FillWidth(1.0)
        [
            SNew(SNumericEntryBox<float>).MinValue(-1000.0f).MaxValue(1000.0f).MinSliderValue(TAttribute< TOptional<float> >(-500.0f)).MaxSliderValue(TAttribute< TOptional<float> >(500.0f)).Delta(0.5f).Value(500.0f).AllowSpin(true)
        ...
        // SNumericEntrySpinBox
        NextSlot(WidgetGrid, LOCTEXT("SNumericEntryBoxNoSpinLabel", "SNumericEntryBox (No Spin)")).FillWidth(1.0)
        [
            SNew(SNumericEntryBox<float>).MinValue(-1000.0f).MaxValue(1000.0f)
        ...
        // SEditableText
        NextSlot(WidgetGrid, LOCTEXT("SEditableTextLabel", "SEditableText")).FillWidth(1.0)
        [
            SNew(SEditableText).HintText(LOCTEXT("SEditableTextHint", "This is editable text"))
        ];

        // SEditableTextBox
        NextSlot(WidgetGrid, LOCTEXT("SEditableTextBoxLabel", "SEditableTextBox")).FillWidth(1.0)
        [
            SNew(SEditableTextBox).HintText(LOCTEXT("SEditableTextBoxHint", "This is an editable text box"))
        ];

        // SMultiLineEditableText
        NextSlot(WidgetGrid, LOCTEXT("SMultiLineEditableTextLabel", "SMultiLineEditableText")).FillWidth(1.0)
        [
            SNew(SBox).MinDesiredHeight(48.f).MinDesiredWidth(200.f)
            [
                SNew(SMultiLineEditableText).HintText(LOCTEXT("SMultiLineEditableTextHint", "This is multi-line \n\t\t\t editable text")).OverflowPolicy(ETextOverflowPolicy::Ellipsis)
            ]
        ];

        // SMultiLineEditableTextBox
        NextSlot(WidgetGrid, LOCTEXT("SMultiLineEditableTextBoxLabel", "SMultiLineEditableTextBox")).FillWidth(1.0)
        [
            SNew(SBox).MinDesiredHeight(48.f).MinDesiredWidth(200.f)
            [
                SNew(SMultiLineEditableTextBox).HintText(LOCTEXT("SMultiLineEditableTextBoxHint", "This is a multi-line editable text box")).OverflowPolicy(ETextOverflowPolicy::Ellipsis)
            ]    
        ];

        // SInlineEditableTextBlock
        NextSlot(WidgetGrid, LOCTEXT("SInlineLineEditableTextLabel", "SInlineEditableText")).FillWidth(1.0)
        [
            SNew(SInlineEditableTextBlock).Text(LOCTEXT("SInlineEditableTextHint", "Inline Editable Text"))
        ];

        // SSearchBox
        NextSlot(WidgetGrid, LOCTEXT("SSearchBoxLabel", "SSearchBox")).FillWidth(1.0)
        [
            SNew(SSearchBox)
        ];

		// SProgressBar
		NextSlot(WidgetGrid, LOCTEXT("SSProgressBar", "SProgressBar")).FillWidth(1.0) 
        [ 
            SNew(SProgressBar).Percent(.5f) 
        ];

		// SProgressBar marquee
		NextSlot(WidgetGrid, LOCTEXT("SSProgressBarMarquee", "SProgressBar marquee")).FillWidth(1.0) 
        [ 
            SNew(SProgressBar) 
        ];
        ...
        [
            SNew(SButton).Text(LOCTEXT("WindowFlashLabel", "Window Flash"))
            .OnClicked_Lambda( [] () -> FReply { FSlateApplication::Get().GetActiveTopLevelWindow()->FlashWindow(); return FReply::Handled(); } )
        ];


		auto NewMenuDelegate = FNewMenuDelegate::CreateLambda([&](FMenuBuilder& MenuBuilder) 
        {
		...
		NextSlot(WidgetGrid, LOCTEXT("MenuBar",   "Menu Bar")) [ MenuBar.MakeWidget() ];
		NextSlot(WidgetGrid, LOCTEXT("Toolbar",    "Toolbar")) [ BuildToolbar(true) ];
		NextSlot(WidgetGrid, LOCTEXT("Toolbar2", "Toolbar 2")) [ BuildToolbar(false) ];

        return SNew(SBorder).BorderImage( FAppStyle::Get().GetBrush("ToolPanel.GroupBorder") )
        [
            SNew(SScrollBox)

            + SScrollBox::Slot().Padding(48.0f)
            [

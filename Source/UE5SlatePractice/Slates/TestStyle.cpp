...
TSharedRef< ISlateStyle > FTestStyle::Create()
{
	...
	// RichText Test Showcase
	{
		...
		const FTextBlockStyle NormalRichTextStyle = FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT("Regular", 14) ).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 )));

		Style->Set( "RichText.Text", NormalRichTextStyle);
		Style->Set( "RichText.TextHighlight", FTextBlockStyle(NormalRichTextStyle).SetColorAndOpacity(FLinearColor(FColor( 0xff19bc9c ))));
		Style->Set( "RichText.Text.Fancy", FTextBlockStyle(NormalRichTextStyle).SetFont( DEFAULT_FONT("BlackItalic", 14) ).SetColorAndOpacity(FLinearColor(FColor( 0xff19bc9c ))) );
		Style->Set( "RichText.Header", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT("Bold", 20) ).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 ))));

		const FButtonStyle DarkHyperlinkButton = FButtonStyle().SetNormal ( BORDER_BRUSH( "Old/HyperlinkDotted", FMargin(0,0,0,3/16.0f), FLinearColor(FColor( 0xff2c3e50 )) ) ) .SetPressed(FSlateNoResource()).SetHovered( BORDER_BRUSH( "Old/HyperlinkUnderline", FMargin(0,0,0,3/16.0f), FLinearColor(FColor( 0xff2c3e50 )) ) );
		const FHyperlinkStyle DarkHyperlink = FHyperlinkStyle().SetUnderlineStyle(DarkHyperlinkButton).SetTextStyle(NormalRichTextStyle).SetPadding(FMargin(0.0f));

		Style->Set( "RichText.Hyperlink", DarkHyperlink );
		Style->Set( "RichText.ImageDecorator", FInlineTextImageStyle().SetImage( IMAGE_BRUSH( "Testing/DefaultPawn_16px", Icon16x16 ) ).SetBaseline( 0 ));
      //Style->Set( "Test.SquiggleArrow", FInlineTextImageStyle().SetImage( IMAGE_BRUSH( "Common/TinyChalkArrow", FVector2D( 71, 20 ))).SetBaseline( -8 ));
		...
		//Tagline
		{
			...
			Style->Set( "RichText.Tagline.Text", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT("Bold", 24) ).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 ))));
			Style->Set( "RichText.Tagline.TextHighlight", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT("Bold", 24) ).SetColorAndOpacity(FLinearColor(FColor( 0xff19bc9c ))) );
			Style->Set( "RichText.Tagline.SubtleText", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 16 ) ).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 ))));
			Style->Set( "RichText.Tagline.SubtleTextHighlight", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 16 )).SetColorAndOpacity(FLinearColor(FColor( 0xff19bc9c ))));
		}
		...
		//Interactive example
		{
			...
			const FTextBlockStyle NormalInteractiveRichText = FTextBlockStyle(NormalText).SetFont(DEFAULT_FONT( "Regular", 12)).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 )));
			Style->Set( "RichText.Interactive.Text", NormalInteractiveRichText	);

			const FButtonStyle HyperlinkButton = FButtonStyle().SetNormal(BORDER_BRUSH( "Old/HyperlinkDotted", FMargin(0,0,0,3/16.0f), FLinearColor(FColor( 0xff2c3e50 )))).SetPressed(FSlateNoResource()).SetHovered( BORDER_BRUSH( "Old/HyperlinkUnderline", FMargin(0,0,0,3/16.0f), FLinearColor(FColor( 0xff2c3e50 )) ) );
			const FHyperlinkStyle Hyperlink = FHyperlinkStyle().SetUnderlineStyle(HyperlinkButton).SetTextStyle(NormalInteractiveRichText).SetPadding(FMargin(0.0f));

			Style->Set( "RichText.Interactive.Text.Hyperlink", Hyperlink );

			// Dialogue
			{
				const FTextBlockStyle NormalInteractiveRichDialogueText = FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Italic", 12)).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 )));
				Style->Set( "RichText.Interactive.Text.Dialogue", NormalInteractiveRichDialogueText);
				Style->Set( "RichText.Interactive.Text.StrongDialogue", FTextBlockStyle(NormalInteractiveRichDialogueText).SetFont( DEFAULT_FONT( "BoldItalic", 12 )).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 ))));

				const FButtonStyle DialogueHyperlinkButton = FButtonStyle().SetNormal ( BORDER_BRUSH( "Old/HyperlinkDotted", FMargin(0,0,0,3/16.0f), FLinearColor(FColor( 0xff2c3e50 )) ) ).SetPressed( FSlateNoResource() ).SetHovered( BORDER_BRUSH( "Old/HyperlinkUnderline", FMargin(0,0,0,3/16.0f), FLinearColor(FColor( 0xff2c3e50 )) ) );
				const FHyperlinkStyle DialogueHyperlink = FHyperlinkStyle().SetUnderlineStyle(DialogueHyperlinkButton).SetTextStyle(NormalInteractiveRichDialogueText).SetPadding(FMargin(0.0f));
				Style->Set( "RichText.Interactive.Text.DialogueHyperlink", DialogueHyperlink );
			}

			// Details
			{
				Style->Set( "RichText.Interactive.Details.Name.Text", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT("Bold", 10) ).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 ))));
				Style->Set( "RichText.Interactive.Details.Value.Text", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 10)).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 ))) );
		...
		// Editor example
		{
			// Default text styles
			{
				...
				{
					const FButtonStyle RichTextHyperlinkButton = FButtonStyle().SetNormal(BORDER_BRUSH("Old/HyperlinkDotted", FMargin(0,0,0,3/16.0f), FSlateColor::UseForeground())).SetPressed(FSlateNoResource() ).SetHovered(BORDER_BRUSH("Old/HyperlinkUnderline", FMargin(0,0,0,3/16.0f), FSlateColor::UseForeground() ) );
					const FHyperlinkStyle RichTextHyperlink = FHyperlinkStyle().SetUnderlineStyle(RichTextHyperlinkButton).SetTextStyle(RichTextNormal).SetPadding(FMargin(0.0f));
			...
			// Toolbar
			{
				...
				Style->Set( "RichText.Toolbar.Text", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 10 ) ).SetColorAndOpacity( TextColor ));
				Style->Set( "RichText.Toolbar.BoldText", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Bold", 10 ) ).SetColorAndOpacity( TextColor ));
				Style->Set( "RichText.Toolbar.ItalicText", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Italic", 10 ) ).SetColorAndOpacity( TextColor ));
...

				{
					...
					const FComboButtonStyle ComboBoxComboButton = FComboButtonStyle(ComboButton).SetButtonStyle(ComboBoxButton).SetMenuBorderPadding(FMargin(1.0));
					Style->Set( "RichText.Toolbar.ComboBox", FComboBoxStyle().SetComboButtonStyle(ComboBoxComboButton));
				}
			}
		}
		...
		{
			Style->Set( "TheWarOfTheWorlds.Text", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 10 ) ).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 )))
			);
		}

		// Rainbow example
		{
			Style->Set( "Rainbow.Text", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 12 )).SetColorAndOpacity(FLinearColor(FColor( 0xff2c3e50 ))));
			Style->Set( "Rainbow.Text.Red", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 12)).SetColorAndOpacity(FLinearColor(FColor( 0xffb40000 ))));
			Style->Set( "Rainbow.Text.Orange", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 12)).SetColorAndOpacity(FLinearColor(FColor( 0xffb46100 ))));
			Style->Set( "Rainbow.Text.Yellow", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 12)).SetColorAndOpacity(FLinearColor(FColor( 0xffb2b400 ))));
			Style->Set( "Rainbow.Text.Green", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 12 )).SetColorAndOpacity(FLinearColor(FColor( 0xff22b400 ))));
			Style->Set( "Rainbow.Text.Blue", FTextBlockStyle(NormalText).SetFont( DEFAULT_FONT( "Regular", 12)).SetColorAndOpacity(FLinearColor(FColor( 0xff006ab4 ))) );
		}
	}

...
class FTestData
{
	...
	public:
		 ...
		 */
		void SetName( const FText& InNewName ) { Name = InNewName; }
		 ...
		 * @param OutChildren  
		 */
		void GetChildren( TArray< TSharedPtr<FTestData> >& OutChildren ) { OutChildren = Children; }

		/** @return the dummy number value */
		float GetNumber() const { return Number; }

		/** @param NewValue  The new dummy value */
		void SetNumber( float NewValue ) { Number = NewValue; }
...
public:
	...
	void Construct(const FArguments& InArgs)
	{
		ChildSlot
		[
			SNew(SBorder).BorderImage(FCoreStyle::Get().GetBrush("Menu.Background")).Padding( FMargin(5) )
			[
				SNew(SVerticalBox)

				+SVerticalBox::Slot().AutoHeight()
				[
					SNew(SButton).Text(LOCTEXT("TestMenuButtonText00", "Option 00")).ToolTipText(LOCTEXT("TestMenuButtonToolTip00", "The first option text."))
				]
				+SVerticalBox::Slot().AutoHeight()
				[
					SAssignNew(PopupAnchor, SMenuAnchor).Placement(MenuPlacement_MenuRight).OnGetMenuContent( this, &STestMenu::OnGetContent )
					[
						SNew(SButton).Text(LOCTEXT("TestMenuButtonText01", "Option 01 >")).ToolTipText(LOCTEXT("TestMenuButtonToolTip01", "The first option text.")).OnClicked( this, &STestMenu::OpenSubmenu )
					]			
				]
				+SVerticalBox::Slot().AutoHeight()
				[
					SNew(SButton).Text(LOCTEXT("TestMenuButtonText02", "Option 02") )
				]
				+SVerticalBox::Slot().AutoHeight()
				[
					SNew(SButton).Text(LOCTEXT("TestMenuButtonText03", "Option 03")).ToolTipText(LOCTEXT("TestMenuButtonToolTip03", "The fourth option text."))
				]
			]
		];
	}
...
public:
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn( const FName& ColumnName ) override
	{
		...
		{
			// The name column is special. In lists it needs to show an expander arrow and be indented in order to give the appearance of a list.
			TSharedRef<SWidget> CellContent = SNew(STextBlock).Font(ItemEditorFont).Text( this, &SItemEditor::OnReadText );
		...
		{
			return SNew(SSpinBox<float>).MinValue(-5).MaxValue(800).Font(ItemEditorFont).Value(this, &SItemEditor::OnReadValue).OnValueChanged(this, &SItemEditor::OnWriteValue);
		}
		else if (ColumnName == TEXT("TextField"))
		{
			return SAssignNew(MyTextBox, SEditableTextBox).Font(ItemEditorFont).Text( this, &SItemEditor::OnReadText).OnTextChanged(this, &SItemEditor::OnTextChanged);
		}
		...
		{
			return SNew(SBorder).HAlign(HAlign_Center).Padding(this, &SItemEditor::GetVariableHeight)
				[
					SNew(SButton).TouchMethod(EButtonTouchMethod::PreciseTap).OnClicked(this, &SItemEditor::OnAddChild)
					[
						SNew(STextBlock).Text( LOCTEXT("AddChildButtonText", "+") )
					]
				];
		}
		else
		{
			return SNew(STextBlock).Text(FText::Format( LOCTEXT("UnsupprtedColumnText", "Unsupported Column: {0}"), FText::FromName(ColumnName)));
		}
	}
	...
	void Construct( const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView )
	{
		...
		FSuperRowType::Construct( FSuperRowType::FArguments().OnCanAcceptDrop(InArgs._OnCanAcceptDrop).OnAcceptDrop(InArgs._OnAcceptDrop).OnDragDetected(InArgs._OnDragDetected).Padding(0), InOwnerTableView);
	}
	
	void SpawnContextMenu( const FVector2D& SpawnLocation )
	{
		FSlateApplication::Get().PushMenu(SharedThis( this ), FWidgetPath(), GetPopupContent(), SpawnLocation, FPopupTransitionEffect( FPopupTransitionEffect::ContextMenu));
	}
...
private:
	...
	float OnReadValue() const { return ItemToEdit->GetNumber(); }
	void  OnWriteValue( float NewValue ) { ItemToEdit->SetNumber(NewValue); }
...
public:
	...
	void Construct( const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView )
	{
		STableRow<TSharedPtr<FTestData> >::ConstructInternal( STableRow::FArguments().OnCanAcceptDrop(InArgs._OnCanAcceptDrop).OnAcceptDrop(InArgs._OnAcceptDrop).OnDragDetected(InArgs._OnDragDetected).ShowSelection(true), InOwnerTableView);
		...
		ChildSlot
		[
			SNew(SBorder).BorderImage( FCoreStyle::Get().GetBrush("ToolPanel.GroupBorder")).BorderBackgroundColor(FLinearColor(1,1,1,0.45f))
			[
				SNew(SVerticalBox)
				...
				[
					SNew(SSpinBox<float>).MinValue(-5).MaxValue(15).Font( ItemEditorFont ).Value( this, &STileItemEditor::OnReadValue).OnValueChanged( this, &STileItemEditor::OnWriteValue )
				]
			    +SVerticalBox::Slot().AutoHeight().Padding(2) 
                [
				    SNew(SEditableTextBox).Font( ItemEditorFont ).Text( this, &STileItemEditor::OnReadText ).OnTextChanged( this, &STileItemEditor::OnTextChanged )
			    ]
				+SVerticalBox::Slot().AutoHeight().Padding(2)
				[
					SNew(STextBlock).Font( ItemEditorFont ) .Text( this, &STileItemEditor::OnReadText )
...
public:
	...
	void Construct( const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView )
	{
		...
		ChildSlot
		[
			SNew(SBorder).BorderImage( FCoreStyle::Get().GetBrush("ToolPanel.GroupBorder")).BorderBackgroundColor(FLinearColor(1,1,1,0.45f)).Padding(this, &SHorizontalListItemEditor::GetVariableWidth)
			[
				SNew(SVerticalBox)
				...
				+SVerticalBox::Slot().AutoHeight().Padding(2)
				[
					SNew(SSpinBox<float>).MinValue(-5).MaxValue(800).Font( ItemEditorFont ).Value( this, &SHorizontalListItemEditor::OnReadValue ).OnValueChanged( this, &SHorizontalListItemEditor::OnWriteValue )
				]
			    +SVerticalBox::Slot().AutoHeight().Padding(2) 
                [
				    SNew(SEditableTextBox).Font( ItemEditorFont ).Text( this, &SHorizontalListItemEditor::OnReadText ).OnTextChanged( this, &SHorizontalListItemEditor::OnTextChanged )
			    ]
				+SVerticalBox::Slot().AutoHeight().Padding(2)
				[
					SNew(STextBlock).Font( ItemEditorFont ) .Text( this, &SHorizontalListItemEditor::OnReadText )
...
public:
		...
		[
			...
			[
				...
				[
					...
					SNew(SNumericEntryBox<int32>).Value( this, &STableViewTesting::GetNumTotalItems).OnValueChanged( this, &STableViewTesting::NumItems_OnValueChanged )
				]
				+SHorizontalBox::Slot().AutoWidth()
				[
					// Press rebuild to clear out the old data items and create the new ones (however many are specified by SEditableTextBox)
					SNew(SButton).Text( LOCTEXT("RebuildDataButtonLabel", "Rebuild Data")).OnClicked( this, &STableViewTesting::Rebuild_OnClicked )
				]
				+SHorizontalBox::Slot().AutoWidth() .Padding(10, 0, 0, 0)
				[
					// We set how many data items we want in our test case.
					SNew(SNumericEntryBox<int32>).Value( this, &STableViewTesting::GetScrollToIndex).OnValueChanged( this, &STableViewTesting::ScrollToIndex_OnValueChanged )
				]
				+SHorizontalBox::Slot().AutoWidth()
				[
					// Press rebuild to clear out the old data items and create the new ones (however many are specified by SEditableTextBox)
					SNew(SButton).Text(LOCTEXT("ScrollToItemButtonLabel", "Scroll to Item")).OnClicked(this, &STableViewTesting::ScrollToIndex_OnClicked)
				]
				...
				+SHorizontalBox::Slot().AutoWidth()
				[
					SNew(SCheckBox).IsChecked(this, &STableViewTesting::IsScrollAnimChecked).OnCheckStateChanged(this, &STableViewTesting::HandleScrollAnimCheckStateChanged)
				]
				...
				+SHorizontalBox::Slot().AutoWidth().Padding(10, 0, 0, 0)
				[
					SNew(SCheckBox).IsChecked(this, &STableViewTesting::IsFixedItemOffsetChecked).OnCheckStateChanged(this, &STableViewTesting::HandleFixedItemOffsetCheckStateChanged)
				]
				+SHorizontalBox::Slot().AutoWidth()
				[
					SNew(SHorizontalBox).IsEnabled(this, &STableViewTesting::GetIsFixedItemOffsetEnabled)
					...
					[
						// We set how many data items we want in our test case.
						SNew(SNumericEntryBox<double>).MinValue(0.).MaxValue(0.5).Value(this, &STableViewTesting::GetFixedItemOffsetValue).OnValueChanged(this, &STableViewTesting::HandleFixedItemOffsetValueChanged)
					]
				]
				+SHorizontalBox::Slot().FillWidth(1)
				[
				...
				[
					...
					[
						SNew(STextBlock).Text( this, &STableViewTesting::GetSelectedModeText)
					]
				]
				+SHorizontalBox::Slot().AutoWidth()
				[
					SNew(SButton).Text( LOCTEXT("RefreshButtonLabel", "Refresh!")).OnClicked( this, &STableViewTesting::RequestRefresh )
				]
			]
			+SVerticalBox::Slot() .AutoHeight()
			[
				...
				+SHorizontalBox::Slot() .AutoWidth() .VAlign(VAlign_Center)
				[
					SNew(STextBlock).Text( LOCTEXT("ExpansionTestingLabel", "Expansion: ") )
				]
				+SHorizontalBox::Slot() .AutoWidth() .VAlign(VAlign_Center)
				[
					SNew(SButton).Text( LOCTEXT("RememberExpansionButton", "Remember")).OnClicked( this, &STableViewTesting::RememberExpansion)
				]
				+SHorizontalBox::Slot() .AutoWidth() .VAlign(VAlign_Center)
				[
					SNew(SButton).Text( LOCTEXT("CollapseExpansionsButton", "Collapse All")).OnClicked( this, &STableViewTesting::CollapseAll)
				]
				+SHorizontalBox::Slot() .AutoWidth() .VAlign(VAlign_Center)
				[
					SNew(SButton).Text( LOCTEXT("RestoreExpansionsButton", "Restore")).OnClicked( this, &STableViewTesting::RestoreExpansion)
				]
			]
			+SVerticalBox::Slot().FillHeight(1)
			[
				SNew(SBox).HeightOverride(800.0f)
				[
					SNew(SSplitter).Orientation(Orient_Vertical)

					+ SSplitter::Slot().Value(.6f)
					[
						SNew( SSplitter )
						...
						[
							...
							[
								...
								[
									...
									(
										SNew(SHeaderRow)
										+ SHeaderRow::Column("Name") [ SNew(SBorder).Padding(5) [ SNew(STextBlock).Text(LOCTEXT("TestNameColumn", "Name"))]]
	...
	STableViewTesting()
        : TotalItems( 5000 ), ScrollToIndex( 0 )
	{
	}
...
protected:
	...
	FText GetSelectedModeText() const
	{
		...
		return (Mode.IsValid()) ? GetSelectedModeText(Mode) : FText::GetEmpty(); 
    }
	...
	{
		if (InMode.IsValid()) { CurSelectionMode = InMode; }
	}
	...
	{
		return SNew(STextBlock).Text( GetSelectedModeText(InMode));
	}
...
TSharedRef<SWidget> MakeTableViewTesting()
{
	...
	return SNew(STableViewTesting).RenderTransform_Static(&GetTestRenderTransform).RenderTransformPivot_Static(&GetTestRenderTransformPivot);
}

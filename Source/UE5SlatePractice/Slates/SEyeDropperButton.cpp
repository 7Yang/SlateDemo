...
void SEyeDropperButton::Construct(const FArguments& InArgs)
{
	...
	SButton::Construct(SButton::FArguments().ContentPadding(1.0f).OnClicked(this, &SEyeDropperButton::OnClicked)
	[
		SNew(SOverlay)

		+ SOverlay::Slot().Padding(FMargin(1.0f, 0.0f))
		[
			SNew(SImage).Image(FCoreStyle::Get().GetBrush("ColorPicker.EyeDropper")).ToolTipText(LOCTEXT("EyeDropperButton_ToolTip", "Activates the eye-dropper for selecting a colored pixel from any window.")) .ColorAndOpacity(this, &SEyeDropperButton::GetDropperImageColor)
		]

		+ SOverlay::Slot()
		[
			SNew(STextBlock).Text(LOCTEXT("EscapeCue", "Esc")).ToolTipText(LOCTEXT("EyeDropperEscapeCue_ToolTip", "Hit Escape key to stop the eye dropper")) .Visibility(this, &SEyeDropperButton::GetEscapeTextVisibility)
		]
	]);
}
...
FReply SEyeDropperButton::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	...
	if ( InKeyEvent.GetKey() == EKeys::Escape && HasMouseCapture() && bWasClickActivated )
	{
		ResetDropperModeStates();
...
FCursorReply SEyeDropperButton::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	...
	if ( HasMouseCapture() && bWasClickActivated && bWasLeft && !bWasReEntered )
	{
